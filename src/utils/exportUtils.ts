...
export function exportQuizAsPDF(quiz: any[], filename: string) {
    ...
    const optionsHTML = q.options.map((opt: string, i: number) => {
      ...
      return `
        <div style="padding: 10px; margin: 6px 0; background: ${isCorrect ? '#dcfce7' : '#f9fafb'}; border-left: 4px solid ${isCorrect ? '#22c55e' : '#e5e7eb'}; border-radius: 4px;">
          ...
          ${isCorrect ? '<span style="color: #16a34a; font-weight: 600; margin-left: 8px;">✓ Correct Answer</span>' : ''}
        </div>
      `;
    }).join('');
  ...
  printWindow.document.write(`
    ...
    <body>
      <h1>📝 Quiz</h1>
      <div class="subtitle">${quiz.length} Questions • Generated on ${new Date().toLocaleDateString()}</div>
      ${questionsHTML}
      <div class="footer">
...
export function exportFlashcardsAsPDF(flashcards: any[], filename: string) {
  ...
  printWindow.document.write(`
    ...
    <body>
      <h1>🎴 Flashcards</h1>
      <div class="subtitle">${flashcards.length} Cards • Generated on ${new Date().toLocaleDateString()}</div>
